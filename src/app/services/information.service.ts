...
export class CompanyService {
  ...
  addCompanyInformation(companyData: Company, id?: string): Observable<any> {
    ...
    return this.http
      .post<{ message: string; id: string }>(url, companyData, {
  ...
  getInformation(
    id?: string
  ): Observable<{ success: boolean; data?: Company; message?: string }> {
    ...
    return this.http.get<Company>(url, { withCredentials: true }).pipe(
      ...
      catchError((error) => {
        const errorMessage =
          error.error?.message || 'Error desconocido al obtener la información';
        return of({ success: false, message: errorMessage });
      })
    );
  }
  ...
  updateCompany(
    ...
    const url = id
      ? `${this.apiUrl}/updateInfo/${id}`
      : `${this.apiUrl}/updateInfo`;

    return this.http
      .put<{ message: string; updatedData: Company }>(url, companyData, {
        withCredentials: true,
      })
      .pipe(
        map((response) => ({
  ...
  deleteDocuments(
    ...
    success: boolean;
    message: string;
    updatedDocuments?: Array<{ fileName: string }>;
  }> {
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          const errorMessage =
            error.error?.message || 'Error desconocido al eliminar documentos';
          return of({ success: false, message: errorMessage });
        })
      );
  }
  ...
  updateDocuments(
    ...
    success: boolean;
    message: string;
    updatedDocuments?: Array<{ fileName: string; url: string }>;
  }> {
    const url = id
      ? `${this.apiUrl}/updateDocuments/${id}`
      : `${this.apiUrl}/updateDocuments`;

    return this.http
      .put<{
        message: string;
        updatedDocuments: Array<{ fileName: string; url: string }>;
      }>(url, formData, { withCredentials: true })
      .pipe(
        map((response) => ({
          success: true,
          message: response.message,
        ...
        catchError((error) => {
          const errorMessage =
            error.error?.message ||
            'Error desconocido al actualizar los documentos';
          return of({ success: false, message: errorMessage });
        })
      );
  }
