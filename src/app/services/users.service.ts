...
export class UserService {
  ...
  createUser(user: FormData): Observable<Usuario | null> {
    return this.http.post<Usuario>(`${this.apiUrl}/register`, user, { withCredentials: true });
  }
 ...
 */
  ...
  getUserById(id?: string): Observable<Usuario | null> {
    ...
    return this.http.get<{ user: Usuario }>(url, { withCredentials: true }).pipe(
  ...
  updateLogo(logo: File, userId?: string): Observable<any> {
    const url = userId ?  `${this.apiUrl}/users/${userId}/logo` : `${this.apiUrl}/users/logo`;
  ...
  updateUser(id: string, updatedData: Partial<Usuario>): Observable<Usuario | null> {
    return this.http.put<Usuario>(`${this.apiUrl}/${id}`, updatedData, { withCredentials: true }).pipe(
      tap(() => this.getAllUsersByType('all')),
      catchError(error => {
