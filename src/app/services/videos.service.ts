...
export class VideoService {
  ...
  addVideo(
    videoUrl: string,
    id?: string
  ): Observable<{ success: boolean; message: string; id?: string; urls?: string[] }> {
    const url = id ? `${this.apiUrl}/add/${id}` : `${this.apiUrl}/add`;

    return this.http.post<{ message: string; id: string; urls: string[] }>(
      url,
      ...
      catchError((error) => {
        const errorMessage = error.error?.message || 'Error desconocido al añadir el video';
  ...
  getVideosByCompanyId(
    id?: string
  ): Observable<{ success: boolean; videos?: Array<{ id: string; companyID: string; urls: string[] }>; message?: string }> {
    const url = id ? `${this.apiUrl}/company/${id}` : `${this.apiUrl}/company`;

    return this.http.get<Array<{ id: string; companyID: string; urls: string[] }>>(url, { withCredentials: true }).pipe(
      map((response) => ({
        success: true,
        videos: response,
      })),
      catchError((error) => {
        const errorMessage = error.error?.message || 'Error desconocido al obtener los videos';
        return of({ success: false, message: errorMessage });
      })
    );
  }
   ...
   */
  deleteVideo(url: string, id?: string): Observable<{ success: boolean; message: string; removedUrl?: string }> {
    const urlEndpoint = id ? `${this.apiUrl}/delete/${id}` : `${this.apiUrl}/delete`;
    const body = { url };

    return this.http.delete<{ message: string; removedUrl: string }>(urlEndpoint, {
      body,
      withCredentials: true,
    }).pipe(
      map((response) => ({
      ...
      catchError((error) => {
        const errorMessage = error.error?.message || 'Error desconocido al eliminar el video';
        return of({ success: false, message: errorMessage });
      })
    );
  }
